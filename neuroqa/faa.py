...
def main():
    ...
    for _, r in manifest.iterrows():
        ...
        result = compute_faa(data, ch_names, sfreq)
        rows.append({
            "file": r["file"], "group": r["group"], "subject": r["subject"],
            "condition": r["condition"], "is_duplicate": bool(r["is_duplicate"]),
            "n_epochs": data.shape[0], "faa": round(result["faa"], 4),
        })
        print(f"  {r['file']:28s} FAA={result['faa']:+.4f}")

    summary = pd.DataFrame(rows)
    out_path = OUT_DIR / "faa_summary.csv"
    summary.to_csv(out_path, index=False)
    print(f"\nwrote {out_path}")
    print(summary.groupby("group").faa.agg(["mean", "std", "count"]).to_string())
